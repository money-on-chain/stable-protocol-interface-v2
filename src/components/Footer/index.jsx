...
function restoreColorSchemePreference() {
  ...
  const defaultScheme = getComputedStyle(document.querySelector(':root')).getPropertyValue("--default-theme").split('"').join('');
...
// Set button and button text based on current selection
  if (isDark) {
    ...
    document.getElementById("SchemeText").innerText=t('settings.themeMode.dark');
  } else {
    colorSchemeSelectorEl.checked = false;
    document.getElementById("SchemeText").innerText=t('settings.themeMode.light');
  }
}
...
const colorSchemeSelectorEl = document.querySelector("#color-scheme-selector");
    ...
    return (
        <div className="dashboard-footer">
            ...
            <div className="lightModeSwitch">
                ...
                <label for="color-scheme-selector"></label>
            </div>
              <div ID="SchemeText" className="lightModeText">Light Mode</div>
            </div>
        </div>
