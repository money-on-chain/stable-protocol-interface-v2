...
const readContracts = async (web3) => {
    ...
    const contractPPCA =
        process.env.REACT_APP_CONTRACT_PRICE_PROVIDER_CA.split(",");
    ...
    console.log(
        `Reading Price Provider ${settings.tokens.COINBASE[0].name} Contract... address: `,
    ...
    if (typeof process.env.REACT_APP_CONTRACT_IREGISTRY !== "undefined") {
        ...
        if (
            typeof process.env.REACT_APP_CONTRACT_INCENTIVE_V2 !== "undefined"
        ) {
            console.log(
    ...
    if (process.env.REACT_APP_CONTRACT_LEGACY_TP) {
        const tpLegacy = new web3.eth.Contract(
            TokenPegged.abi,
            process.env.REACT_APP_CONTRACT_LEGACY_TP
        );
        dContracts.contracts.tp_legacy = tpLegacy;

        if (!process.env.REACT_APP_CONTRACT_TOKEN_MIGRATOR)
            console.log("Error: Please set token migrator address!");

        const tokenMigrator = new web3.eth.Contract(
            TokenMigrator.abi,
            process.env.REACT_APP_CONTRACT_TOKEN_MIGRATOR
        );
