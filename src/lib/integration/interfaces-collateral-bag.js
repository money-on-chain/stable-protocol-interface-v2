...
const redeemTC = async (interfaceContext, caIndex, qTC, limitAmount, onTransaction, onReceipt) => {
    ...
    console.log(`Redeeming ${qTC} ${settings.tokens.TC.name} ... getting approx limit down to: ${limitAmount} ${settings.tokens.CA[caIndex].name}... `)
    const userTCBalance = new BigNumber(fromContractPrecisionDecimals(userBalanceData.TC.balance,
    ...
    const tcAvailableToRedeem = new BigNumber(Web3.utils.fromWei(contractStatusData.getTCAvailableToRedeem))
    if (new BigNumber(qTC).gt(tcAvailableToRedeem))
        throw new Error(`Insufficient ${settings.tokens.TC.name}available to redeem in contract`)

    // There are sufficient CA in the contract
    const caBalance = new BigNumber(fromContractPrecisionDecimals(contractStatusData.getACBalance[caIndex],
        settings.tokens.CA[caIndex].decimals))
    if (new BigNumber(limitAmount).gt(caBalance))
        throw new Error(`Insufficient ${settings.tokens.CA[caIndex].name} in the contract. Balance: ${caBalance} ${settings.tokens.CA[caIndex].name}`)
...
const mintTP = async (interfaceContext, caIndex, tpIndex, qTP, limitAmount, onTransaction, onReceipt) => {
    ...
    // User have sufficient reserve to pay?
    console.log(`To mint ${qTP} ${settings.tokens.TP[tpIndex].name} you need > ${limitAmount.toString()} ${settings.tokens.CA[caIndex].name} in your balance`)
    const userReserveBalance = new BigNumber(fromContractPrecisionDecimals(userBalanceData.CA[caIndex].balance, settings.tokens.CA[caIndex].decimals))
    if (limitAmount.gt(userReserveBalance))
        throw new Error(`Insufficient ${settings.tokens.CA[caIndex].name} balance`)

    // Allowance
    console.log(`Allowance: To mint ${qTP} ${settings.tokens.TP[tpIndex].name} you need > ${limitAmount.toString()} ${settings.tokens.CA[caIndex].name} in your spendable balance`)
    const userSpendableBalance = new BigNumber(fromContractPrecisionDecimals(userBalanceData.CA[caIndex].allowance, settings.tokens.CA[caIndex].decimals))
    ...
    const tpAvailableToMint = new BigNumber(fromContractPrecisionDecimals(contractStatusData.getTPAvailableToMint[tpIndex], settings.tokens.TP[tpIndex].decimals))
    if (new BigNumber(limitAmount).gt(tpAvailableToMint))
        throw new Error(`Insufficient ${settings.tokens.TP.name} available to mint`)
    ...
    const estimateGas = await MocCAWrapper.methods
        .mintTP(
            caAddress,
            tpIndex,
            toContractPrecisionDecimals(new BigNumber(qTP), settings.tokens.TP[tpIndex].decimals),
            toContractPrecisionDecimals(limitAmount, settings.tokens.CA[caIndex].decimals)
        ).estimateGas({ from: account, value: '0x' })
...
const redeemTP = async (interfaceContext, caIndex, tpIndex, qTP, limitAmount, onTransaction, onReceipt) => {
    ...
    console.log(`Redeeming ${qTP} ${settings.tokens.TP[tpIndex].name} ... getting approx: ${limitAmount} ${settings.tokens.CA[caIndex].name}... `)
    const userTPBalance = new BigNumber(fromContractPrecisionDecimals(userBalanceData.TP[tpIndex], settings.tokens.TP[tpIndex].decimals))
    if (new BigNumber(qTP).gt(userTPBalance))
        throw new Error(`Insufficient ${settings.tokens.TP[tpIndex].name}  user balance`)

    // There are sufficient Free Pegged Token in the contracts to redeem?
    const tpAvailableToRedeem = new BigNumber(Web3.utils.fromWei(contractStatusData.getTPAvailableToMint[tpIndex]))
    if (new BigNumber(qTP).gt(tpAvailableToRedeem))
        throw new Error(`Insufficient ${settings.tokens.TP[tpIndex].name}  available to redeem in contract`)

    // There are sufficient CA in the contract
    const caBalance = new BigNumber(fromContractPrecisionDecimals(contractStatusData.getACBalance[caIndex], settings.tokens.CA[caIndex].decimals))
    if (new BigNumber(limitAmount).gt(caBalance))
        throw new Error(`Insufficient ${settings.tokens.CA[caIndex].name} in the contract. Balance: ${caBalance} ${settings.tokens.CA[caIndex].name}`)
